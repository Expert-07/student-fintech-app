...
exports.setBudget = async (req, res) => {
    ...
    try {
        //Check for existiing entry
        const existing = await db.query(`SELECT * FROM budgets WHERE user_id = $1 AND category = $2`, [user_id, category]);

        let result;
        if (existing.rows.length > 0) {
            const result = await db.query(`UPDATE budgets SET limit_amount = $1 WHERE user_id = $2 AND category = $3 RETURNING *`, [limit_amount, user_id, category]);
        ...
        console.error('Error setting budgetF:', err);
...
exports.getBudget = async (req, res) => {
    ...
    try {
        ...
        console.error('ErrorfetchingBudgets:', error);
        res.status(500).json({ message: 'ServerError' });
    }
};
